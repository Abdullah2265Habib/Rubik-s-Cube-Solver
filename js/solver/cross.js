/**
 * js/solver/cross.js
 * CFOP White Cross Solver Engine with memory-efficient search.
 */
...
function solveCross(initialCube) {
    ...
    // 1. Fast BFS with state deduplication up to depth 6
    const bfsMoves = findCrossBFS(cube, 6);
    if (bfsMoves !== null) {
        return optimizeMoves(bfsMoves);
    }

    // 2. Rule-based piece placement fallback
    const ruleMoves = [];
    const targetEdges = [
...
/**
 * Memory-efficient BFS search with visited set.
 */
